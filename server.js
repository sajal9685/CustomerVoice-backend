...
app.put("/products/:id", async (req, res) => {
  try {
    ...
    const { title, price, description, category, image } = req.body;
    const sql =
      "UPDATE users SET title = ?, price = ?, description = ?, category = ?, image = ? WHERE id = ?";
    const [result] = await db.execute(sql, [
      title,
      price,
      description,
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ id: result.insertId, email, password, first_name, last_name });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error.message });
  }
...
app.get("/reviews/:id/average", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching average rating", error: err.message });
  }
});
...
app.post("/reviews", async (req, res) => {
  try {
    ...
    if (!user_id || !rating || !product_id) {
      return res
        .status(400)
        .json({ error: "Fields (user_id, rating, product_id) are required." });
    }
    ...
    const sql =
      "INSERT INTO reviews (user_id, text, rating, product_id) VALUES (?, ?, ?, ?)";
    const [result] = await db.execute(sql, [user_id, text, rating, product_id]);

    res
      .status(201)
      .json({ id: result.insertId, user_id, text, rating, product_id });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
